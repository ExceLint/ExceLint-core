export class InfoGain {
  ...
  public static to_histogram(data: Array<any>): Map<number, number> {
    const arr = (data as any).flat(Infinity);
    const hist = arr.reduce(
...
export class Stencil {
  ...
  private static ninePointStencil: Array<[number, number]> = [
    [-1, -1],
    [-1, 0],
    [-1, 1],
    [0, -1],
  ...
  public static initialize(): void {
    if (!Stencil.initialized) {
      ...
      if (Stencil.reflectStencils) {
        ...
        reflected = Stencil.stencil_top
          .filter(([x, _]) => x < 0)
          .map(([x, y]) => [-x, y]);
        Stencil.stencil_topleft = Stencil.stencil_topleft.concat(reflected);
        reflected = Stencil.stencil_left
          .filter(([_, y]) => y < 0)
          .map(([x, y]) => [x, -y]);
        Stencil.stencil_topleft = Stencil.stencil_topleft.concat(reflected);
        // Remove [1,1]
        Stencil.stencil_topleft.splice(
          Stencil.stencil_topleft.findIndex(
            o => JSON.stringify(o) === JSON.stringify([1, 1])
          ),
          1
        );
        ...
        reflected = Stencil.stencil_right
          .filter(([_, y]) => y < 0)
          .map(([x, y]) => [x, -y]);
        Stencil.stencil_topright = Stencil.stencil_topright.concat(reflected);
        // Remove [-1,1]
        Stencil.stencil_topright.splice(
          Stencil.stencil_topright.findIndex(
            o => JSON.stringify(o) === JSON.stringify([-1, 1])
          ),
          1
        );
        ...
        reflected = Stencil.stencil_bottom
          .map(([x, y]) => [-x, y])
          .filter(([x, _]) => x > 0);
        Stencil.stencil_bottomleft = Stencil.stencil_bottomleft.concat(
          reflected
        );
        reflected = Stencil.stencil_left
          .map(([x, y]) => [x, -y])
        ...
        // Remove [1,-1]
        Stencil.stencil_bottomleft.splice(
          Stencil.stencil_bottomleft.findIndex(
            o => JSON.stringify(o) === JSON.stringify([1, -1])
          ),
          1
        );
        ...
        reflected = Stencil.stencil_bottom
          .map(([x, y]) => [-x, y])
          .filter(([x, _]) => x < 0);
        Stencil.stencil_bottomright = Stencil.stencil_bottomright.concat(
          reflected
        );
        reflected = Stencil.stencil_right
          .map(([x, y]) => [x, -y])
        ...
        // Remove [-1,-1]
        Stencil.stencil_bottomright.splice(
          Stencil.stencil_bottomright.findIndex(
            o => JSON.stringify(o) === JSON.stringify([-1, -1])
          ),
          1
        );
  ...
  private static apply_stencil(
    stencil: Array<[number, number]>,
  ...
  public static stencil_computation(
    arr: Array<Array<number>>,
    operator: (a: number, b: number) => number,
    base: number
  ): Array<Array<number>> {
    ...
    for (let i = 1; i < ncols - 1; i++) {
      for (let j = 1; j < nrows - 1; j++) {
        //                console.log('i = ' + i + ', j = ' + j);
        new_arr[j][i] = Stencil.apply_stencil(
          Stencil.stencil,
          arr,
          i,
          j,
    ...
    for (let j = 1; j < ncols - 1; j++) {
      //            console.log('top');
      new_arr[0][j] = Stencil.apply_stencil(
        Stencil.stencil_top,
        arr,
        0,
        j,
      ...
      //           console.log('bottom');
      new_arr[nrows - 1][j] = Stencil.apply_stencil(
        Stencil.stencil_bottom,
        arr,
        nrows - 1,
        j,
        base,
    ...
    for (let i = 1; i < nrows - 1; i++) {
      new_arr[i][0] = Stencil.apply_stencil(
        Stencil.stencil_left,
        arr,
        i,
        0,
        base,
      ...
      new_arr[i][ncols - 1] = Stencil.apply_stencil(
        Stencil.stencil_right,
        arr,
        i,
        ncols - 1,
        base,
    ...
    //  console.log('new_arr = ' + JSON.stringify(new_arr));
    // Corners
    new_arr[0][0] = Stencil.apply_stencil(
      Stencil.stencil_topleft,
      arr,
      0,
      0,
      base,
    ...
    new_arr[0][ncols - 1] = Stencil.apply_stencil(
      Stencil.stencil_topright,
      arr,
      0,
      ncols - 1,
      base,
    ...
    new_arr[nrows - 1][0] = Stencil.apply_stencil(
      Stencil.stencil_bottomleft,
      arr,
      nrows - 1,
      0,
      base,
    ...
    new_arr[nrows - 1][ncols - 1] = Stencil.apply_stencil(
      Stencil.stencil_bottomright,
      arr,
      nrows - 1,
      ncols - 1,
      base,
