...
export class FixDiff {
    ...
    public compute_fix_diff(str1, str2, c1, r1, c2, r2) { //c2, r2) {
        ...
        // Build up the diff.
        let difference = diff.main(rc_str1, rc_str2);
        let theDiff = [[...difference], [...difference]];
...
function showDiffs(str1, row1, col1, str2, row2, col2) {
    let nd = new FixDiff();
    let [diff0, diff1] = nd.compute_fix_diff(str1, str2, col1 - 1, row1 - 1, col2 - 1, row2 - 1);
    console.log(diff0);
    console.log(diff1);
    // console.log(JSON.stringify(diffs));
    let [first, dummy2] = nd.pretty_diffs(diff0);
    let [dummy1, second] = nd.pretty_diffs(diff1);
...
let str1 = '=ROUND(E$10:C9)'; // 'ROUNDUP(B2)+12';
let str2 = '=ROUND(E10:B9)'; // 'ROUND(A1)+12';
