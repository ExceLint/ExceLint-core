  ...
  const args = yargs(process.argv)
    ...
    .command(
      ...
      "Set discount for formatting differences (default = " +
        conf.defaultFormattingDiscount +
        ")."
    )
    .command(
      "reportingThreshold",
      "Set the threshold % for reporting anomalous formulas (default = " +
        conf.defaultReportingThreshold +
        ")."
    )
    ...
    .command(
      ...
      "Maximum number of categories for reported errors (default = " +
        conf.defaultMaxCategories +
        ")."
    )
    .command(
      "minFixSize",
      "Minimum size of a fix in number of cells (default = " +
        conf.defaultMinFixSize +
        ")"
    )
    .command(
      "maxEntropy",
      "Maximum entropy of a proposed fix (default = " +
        conf.defaultMaxEntropy +
        ")"
    )
